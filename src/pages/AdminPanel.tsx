...
const AdminPanel = () => {
  ...
  const handleClearOldNotices = async () => {
    // Get notices older than 30 days
    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
    
    const oldNotices = notices.filter(
      notice => new Date(notice.createdAt) < thirtyDaysAgo
    );
    ...
    for (const notice of oldNotices) {
      try {
        ...
        console.error(`Failed to delete notice ${notice.id}:`, error);
      }
    }
  };
                ...
                <div className="bg-card border rounded-lg p-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-start gap-2">
                      <AlertCircle className="h-5 w-5 text-primary mt-0.5" />
                      <p className="text-sm">
                        The "Clear Old Notices" button will remove all notices older than 30 days from the system.
                      </p>
                    </div>
                  </div>
                </div>
